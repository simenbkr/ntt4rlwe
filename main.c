...
static uint64_t modmul(uint64_t a, uint64_t b, uint64_t NTT_MODULUS) {
    return  (((__uint128_t) a * (__uint128_t) b) % (__uint128_t) NTT_MODULUS);
}


uint64_t inverse(uint64_t x, uint64_t y) {
    uint64_t r, s, q, t, p, pos;
    if (y != 0) x %= y;
    r = 1;
    s = 0;
    p = y;
...
uint64_t modexp(uint64_t a, uint64_t exp, uint64_t modulus) {
    ...
    __uint128_t base = modq_64(a, modulus);
...
int is_primitive_root(uint64_t x, uint64_t n, uint64_t q) {
    uint64_t i;

    for (i=1; i<n; i++) {
        if (modexp(x, i, q) == 1) {
            printf("NOT A PRIMITIVE ROOT\n");
            return 0;
        }
}

    if(modexp(x, n , q) == 1) {
        printf("PRIMITIVE ROOT, WOOO");
        return 1;
    }
...
// Kinda stolen from BLISS. No shame.
...
int main() {
    ...
    build_rev_table(omegas_inv, DIMENSION, MODULUS, 1, phi_inv);

    printf("uint64_t phi = %lu;\n", phi);
    printf("uint64_t phi_inv = %lu;\n", phi_inv);
    printf("uint64_t N_INVERSE = %lu;\n", N_INVERSE);

    printf("const uint64_t omega[%lu] = {", DIMENSION);
    for (int i = 0; i < DIMENSION; ++i) {
        printf("%lu, ", omegas[i]);
    }
    printf("};\n\n");

    printf("const uint64_t omega_inv[%lu] = {", DIMENSION);
    for (int i = 0; i < DIMENSION; ++i) {
        printf("%lu, ", omegas_inv[i]);
    }
    printf("};");
